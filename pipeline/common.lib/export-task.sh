...
sed -i "s/<TAG>/${VERSION}/" tn-assetexport-job.yaml
sed -i "s/<TAG>/${VERSION}/" tn-importexportscript-cm.yaml
...
if [ -f "ExportedData-${VERSION}.zip" ]; then
    ...
    unzip ExportedData-${VERSION}.zip -y #unzip the content to make it available for deployment tasks
